...
// Get user's events (created or invited to)
...
router.get('/:eventId', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Check if user has access (creator or invited)
  ...
  async (req, res) => {
    ...
    try {
      await transaction(async (client) => {
        ...
        // Remove creator from invites (they're automatically part of it)
        ...
        if (userIdsToInvite.size > 0) {
          ...
          const validFriendIds = new Set(friendCheck.rows.map(r => r.friend_id));
          
          // Only invite valid friends
          for (const userId of userIdsToInvite) {
            if (validFriendIds.has(userId)) {
              await client.query(
